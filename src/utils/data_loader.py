...
class CordeauParser:
    ...
    @classmethod
    def read_data(cls, path: str) -> CordeauInstance:
        p = Path(path)
        if not p.exists():
        ...
        for line in node_lines:
            ...
            combinations = [
                VisitCombination(code=int(token), days=cls._code_to_days(int(token), depot_count))
    ...
    @classmethod
    def read_solution(cls, path: str) -> CordeauSolution:
        p = Path(path)
        if not p.exists():
...
def read_cordeau_data_file(path: str) -> CordeauInstance:
    return CordeauParser.read_data(path)


def read_cordeau_solution_file(path: str) -> CordeauSolution:
    return CordeauParser.read_solution(path)
